...
\section{Overhead}
    ...
    \begin{equation}
        \begin{array}{ l }
            ...
            \displaystyle T_{px}=h_a \cdot v_a \cdot (\frac{5}{2 \cdot 16} + 1) \\[13pt]
            \displaystyle T_{px}=h_a \cdot v_a \cdot (\frac{5}{2 \cdot 32} + 1) \\[13pt]
        \end{array}
        \label{eq:total_pixel2}
    \end{equation}

    Computing the overhead percentage, $O_{pdp}$, is a matter of dividing the total pixel overhead, $O_{px}$, by the total pixels, $T_{px}$.
    ...
    Due to the nature of the total pixel overhead being proportional to the visible pixel size for each resolution, the overhead percentage under worst-case conditions cancels out to a constant consisting of only a relationship between the minimum packet size and packet overhead. The overhead for packed data is $13.5\%$ and the overhead for unpacked data is $7.2\%$.
    ...
    \begin{equation}
        \begin{array}{ l }
            ...
            \displaystyle O_{pdp}=\frac{5}{5 + 2 \cdot 16} \cdot 100 \\[19pt]
            \displaystyle O_{pdp}=13.5 \\[13pt]
